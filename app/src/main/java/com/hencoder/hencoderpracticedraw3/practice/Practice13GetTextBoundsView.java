...
public class Practice13GetTextBoundsView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int offset1 = - (bound.top + bound.bottom) / 2;

        paint2.getTextBounds(text2,0,1,bound);
        int offset2 = - (bound.top + bound.bottom) / 2;

        paint2.getTextBounds(text3,0,1,bound);
        int offset3 = - (bound.top + bound.bottom) / 2;

        paint2.getTextBounds(text4,0,1,bound);
        int offset4 = - (bound.top + bound.bottom) / 2;

        paint2.getTextBounds(text5,0,1,bound);
        int offset5 = - (bound.top + bound.bottom) / 2;

        paint2.getTextBounds(text6,0,1,bound);
        int offset6 = - (bound.top + bound.bottom) / 2;
